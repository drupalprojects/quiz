...
class ScaleQuestion extends QuizQuestion {
  ...
  public function saveAnswerCollection($is_new_node, array $alt_input = NULL, $preset = NULL) {
    ...
    $alternatives = array();
    if ($alt_input) {
      for ($i = 0; $i < $config->get('scale_max_num_of_alts'); $i++) {
  ...
  public function getNodeProperties() {
    ...
    $res = db_query('SELECT id, answer, a.answer_collection_id
            ...
            ORDER BY a.id', array(':nid' => $this->node->id(), ':vid' => $this->node->getRevisionId()));
  ...
  public function getNodeView() {
    ...
    $alternatives = array();
    for ($i = 0; $i < \Drupal::config('scale.settings')->get('scale_max_num_of_alts'); $i++) {
  ...
  public function getCreationForm(array &$form_state = NULL) {
    ...
    $max_num_alts = \Drupal::config('scale.settings')->get('scale_max_num_of_alts');
